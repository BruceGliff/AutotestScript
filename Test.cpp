...
ErrorType Launch()
{
    ...
    {
        std::cout << std::setfill('-') << std::setw(40) << std::left << x.first;
        if (input.find(x.first) == input.end())
        {
            std::stringstream command;
            command << "./program " << x.second << " " 
        ...
        if(ret != ErrorType::Ok)
        {
            ...
            errout << '"' << x.first << '"' << " doesn't found in check directory!";
...
int main()
{
    std::cout << "Testing is in process..." << std::endl;
    switch (Launch())
    {
